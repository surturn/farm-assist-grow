...
interface Notification {
    ...
    read: boolean;
    createdAt?: string | { toDate: () => Date } | { _seconds: number, _nanoseconds: number };
}
                                            ...
                                            <div className="flex-1 space-y-1">
                                                <div className="flex items-start justify-between">
                                                    ...
                                                    <span className="text-xs text-muted-foreground flex items-center gap-1 whitespace-nowrap ml-2">
                                                        ...
                                                        {note.createdAt && typeof note.createdAt === 'object' && '_seconds' in note.createdAt 
                                                            ? format(new Date(note.createdAt._seconds * 1000), 'MMM d, h:mm a') 
                                                            : note.createdAt && typeof note.createdAt === 'object' && 'toDate' in note.createdAt 
                                                                ? format(note.createdAt.toDate(), 'MMM d, h:mm a')
                                                                : note.createdAt ? format(new Date(note.createdAt as string), 'MMM d, h:mm a') : 'Just now'}
                                                    </span>
                                                </div>
